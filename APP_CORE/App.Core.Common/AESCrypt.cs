...
public class AESCrypt
{
    ...
    private readonly AESCryptOptions Options;
    ...
    ///     Passphrase (in string format) from which a pseudo-random password will be derived. The derived password will be
    ///     used to generate the encryption key.
    /// </param>
    /// <remarks>
    ///     This constructor is NOT recommended because it does not use initialization vector and uses the ECB cipher mode,
    ///     which is less secure than the CBC mode.
    /// </remarks>
    ...
    /// <summary>
    ///     Use this constructor to perform encryption/decryption with the following options:
    ///     - 128/192/256-bit key (depending on passPhrase length in bits) 32 = 256
    ///     - SHA-1 password hashing algorithm with 4-to-8 byte long password hash salt and 1 password iteration
    ///     - hashing without salt
    ...
    ///     Initialization vector (IV). This value is required to encrypt the first block of plaintext data. IV must be exactly
    ...
    /// </param>
    /// <param name="options">
    ///     A set of customized (or default) options to use for the encryption/decryption: see AESCryptOptions documentation
    ///     for details.
    /// </param>
    public AESCrypt(string passPhrase, string initVector, AESCryptOptions options)
    {
        ...
        if (Options.FixedKeySize.HasValue
            ...
            throw new NotSupportedException(
                "ERROR: options.FixedKeySize must be NULL (for auto-detect) or have a value of 128, 192 or 256");
        ...
        else
        {
            if (initVector.Length != 16)
                throw new NotSupportedException("IV must 16 byte, dcm!");
            initVectorBytes = Encoding.UTF8.GetBytes(initVector);
        }

        // Gets the KeySize
        var keySize = Options.FixedKeySize.HasValue
        ...
        lock (this)
        {
            ...
            using (var memoryStream = new MemoryStream())
            {
                ...
                using (var cryptoStream = new CryptoStream(memoryStream, encryptor, CryptoStreamMode.Write))
                {
                    ...
                    cryptoStream.FlushFinalBlock(); // Move encrypted data from memory into a byte array.
    ...
    private byte[] AddSalt(byte[] plainTextBytes)
    {
        ...
        // Generate the salt.
        var saltBytes = GenerateSalt(Options.MinSaltLength, Options.MaxSaltLength);
