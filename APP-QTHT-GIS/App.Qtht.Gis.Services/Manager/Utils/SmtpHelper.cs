...
using System.Net;
using System.Text;
using App.Qtht.Api.Utils.SmtpConnector;

namespace App.Qtht.Api.Utils;
...
public class SmtpHelper
{
    public static bool ValidateCredentials(string login, string password, string server, int port, bool enableSsl)
    {
        ...
        if (!connector.CheckResponse(220)) return false;

        connector.SendData($"HELO {Dns.GetHostName()}{SmtpConnectorBase.EOF}");
        if (!connector.CheckResponse(250)) return false;

        connector.SendData($"AUTH LOGIN{SmtpConnectorBase.EOF}");
        if (!connector.CheckResponse(334)) return false;
        ...
        if (!connector.CheckResponse(235)) return false;
